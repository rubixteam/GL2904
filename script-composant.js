...
////////////////////////////////////Envoi de la requete//////////////////////////////////////////////
		...
		xmlHttpRequest.open("GET", "inscription-composant.php?verif=true&login="+loginRec, true);//etat : 1
...
function checkLogin(){
	if(xmlHttpRequest.readyState == 4){
		try{
			...
			if(tabLogins.item(0)){
				window.document.getElementById("mention").innerHTML = "Composant Existant";
...
function checkValidate(){
	...
	if (checkStateRec == "true"){
		...
		var typeRec=document.getElementById("typeform").value;
		var natureRec=document.getElementById("natureform").value;
		var licenceRec=document.getElementById("licenceform").value;
		var coutRec=document.getElementById("montantform").value;
		xmlHttpRequest.open("GET", "inscription-composant.php?verif=false&login="+loginRec+"&pwd="+pwdRec+"&type="+typeRec+"&nature="+natureRec+"&licence="+licenceRec+"&cout="+coutRec, true);
        xmlHttpRequest.send(null); 
/////////////////////////////Pour définir le traitement de la réponse du serveur///////////////////////		
        xmlHttpRequest.onreadystatechange = validateResponse;
    }		
	}else{
		alert('Composant Existant');
	}
}
